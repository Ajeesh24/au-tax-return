...
- **Capital gains tax (CGT)** — gain/loss with the 50% discount and correct loss ordering
...
**One command** — installs the skill into your Claude skills directory:

```bash
# Current user (~/.claude/skills/au-tax-return)
...
**Manual** — copy the `au-tax-return/` folder into your Claude skills directory, or
point Claude at `au-tax-return/SKILL.md`.
...
- **Generic & reusable** — no user, bank, vendor, or document is hardcoded; works for
  any taxpayer and any set of documents.
- **Year-aware** — it asks which income year the return is for and applies the law and
  ...
  `au-tax-return/references/rates.md` + `au-tax-return/scripts/rates.json`).
...
- **Safe** — never invents numbers, maximises only *legitimate* benefit, and reminds
  you to verify against ATO prefill.

## Repository layout

```
.
├── package.json                    # npx installer entry
├── bin/cli.js                      # zero-dependency Node installer
└── au-tax-return/                  # the skill itself
    ...
    └── scripts/
        ├── rates.json              # machine-readable rates per year
...
```bash
python3 au-tax-return/scripts/tax_calc.py --year 2025-26 --salary 90000 \
    --deductions 3200 --net-rental -4200 --net-capital-gain 1500 --withheld 22000 --cover
```
...
before lodging.** This project does not lodge returns and is **not tax advice**, and is
not a registered tax agent. For anything complex (trusts, businesses, foreign income,
complex CGT), see a registered tax agent.
